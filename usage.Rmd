...
For site-scale simulations, see [here](./input_sitelevel.html), for global simulations, see [here](./input_global.html).

### 3. Get site and simulation parameter files

For site-level simulations, a separate simulation is done for each site. Hence, simulation and site parameter files are needed for each site and simulation.
...
Both are created by `prepare_paramfils.R` available from the repository [getin](https://bitbucket.org/labprentice/getin). In the header adjust manually (the example given here is for site-level simulations at FLUXNET 2015 sites)
```{r, eval=FALSE}
##--------------------------------------------------------------------
## MANUAL SETTINGS
##--------------------------------------------------------------------
myhome           = "your_chosen_home"
